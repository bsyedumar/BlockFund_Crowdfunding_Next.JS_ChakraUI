...
import { getOpenCampaigns } from '../smart-contract/campaigns';
...
export async function getServerSideProps(context) {
  try {
    const openCampaigns = await getOpenCampaigns(1337);
    console.log('campaigns before filtering:', openCampaigns); // Update this line

    console.log("openCampaigns in getServerSideProps:", openCampaigns);

    return {
      props: { campaigns: openCampaigns || [] },
    };
  } catch (error) {
...
function CampaignCard({
  ...
  return (
    <NextLink href={`/campaign/${id.address}`}>
      <Box
          ...
          <Flex direction="row" py={2}>
            <Box w="full">
              ...
              <Text fontSize={"md"} fontWeight="normal">
                target of {target ? ethers.utils.formatUnits(target, "ether") : "0"} ETH ($
                {getWEIPriceInUSD(ethPrice, target)})
              </Text>
              <Progress
                colorScheme="teal"
                size="sm"
                value={balance ? ethers.utils.formatUnits(balance, "ether") : "0"}
                max={target ? ethers.utils.formatUnits(target, "ether") : "0"}
                mt="2"
              />
  ...
  async function getSummary() {
    try {
      const summary = await Promise.all(
        campaigns.map(
          async (campaign) =>
            await Campaign(campaign.address).methods.getSummary().call()
        ),
      );
                ...
                return (
                  <div key={i}>
                    <CampaignCard
                      ...
                      target={el[8]}
                      balance={el[1]}
                      ethPrice={ethPrice} // Add conditional check here
                    />
                  </div>
                );
              })}
